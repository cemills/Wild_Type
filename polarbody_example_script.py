...
# code requires you to have a csv file in subfolder "data" that contains OD600
# data (1 cm PL) for your strain of interest as a function of time (hr)
GC_ODs_N = pd.read_csv("data/GC_ODs_N.csv")
Time = GC_ODs_N.loc[:,'Time'].astype(np.float64)

# log transform and fit
WT_a_log10 = np.log10(GC_ODs_N.loc[:, 'dB_a'])
...
# create model: Enzymes in polar bodies

# MCP geometry
radius_mcp = 7e-8 * (15**(1/3))     # set to keep the total enzyme volume the same.
...
# assumes that enzyme concentration in polar body is same as in MCPs
rmcp_eff = radius_mcp     # [=] m
vmcp_eff = (4/3)*np.pi*(rmcp_eff**3)    # [=] m^3
NAvogadro = 6.02e23

# MCP || PduCDE || forward
kcatCDE = 0.2*300.   # [=] 1/s
N_CDE = 400.*15     # [=] enzymes per compartment
CDE_con = N_CDE / (NAvogadro * vmcp_eff)   # [=] mM
VmaxCDEf = kcatCDE * CDE_con # [=] mM/s

# MCP || PduP || forward
kcatPf = 55.    # [=] 1/s
N_P = 200.*15      # [=] enzymes per compartment
...
# MCP || PduQ || forward
kcatQf = 55.    # [=] 1/s
N_Q = 150.*15      # [=] enzymes per compartment
...
params = {'PermMCPPropanediol': PermMCPPolar,
            ...
            'nmcps': nmcp,
            'PermCellPropanediol': 10**-5,
            'PermCellPropionaldehyde': 10**-5,
            'PermCellPropanol': 10**-5,
            'PermCellPropionyl': 10**-5,
            'PermCellPropionate': 10**-5,
            'VmaxCDEf': VmaxCDEf,
